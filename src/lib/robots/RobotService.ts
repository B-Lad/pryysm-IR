...
export class RobotService {
  ...
  static async createTask(taskData: {
    robotId: string;
    printerId: string;
    taskType: 'PART_REMOVAL' | 'BED_PREP' | 'INSPECTION' | 'MATERIAL_LOAD' | 'BUILD_PLATFORM_REMOVAL' | 'DOOR_OPEN' | 'DOOR_CLOSE' | 'BASKET_TRANSFER' | 'WASH_LOAD' | 'WASH_UNLOAD' | 'CURE_LOAD' | 'CURE_UNLOAD' | 'DENTAL_MODEL_INSPECTION' | 'PLATFORM_CLEANING' | 'RESIN_REFILL';
    priority?: number;
    parameters?: any;
  }): Promise<RobotTask> {
  ...
  static async emergencyStopAll(): Promise<void> {
    ...
    // Note: Emergency stop events are logged by the calling API route with user context.
    // Do not double-log here to avoid duplicate EMERGENCY_STOP records.
  }
